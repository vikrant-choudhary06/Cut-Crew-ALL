...
router.get('/:imdb_id', async (req, res, next) => {
  try {
    ...
    // Call 3: Request the final stream playable links from provider
    // Using exact type (movie/series) instead of generic logic
    let providerUrl = `${process.env.PROVIDER_URL}/api/${provider}/stream?link=${encodeURIComponent(targetLink || imdb_id)}&type=${contentType}&imdbId=${imdb_id}`;
    
    // Set timeout
    const response = await axios.get(providerUrl, { timeout: 30000 });
