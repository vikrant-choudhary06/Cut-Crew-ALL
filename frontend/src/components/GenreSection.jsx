...
const GenreSection = ({ title, genreQuery, badgeColor, badgeText }) => {
  ...
  if (loading) {
    return (
      <div className="mt-12 w-full animate-pulse">
        <h2 className="text-3xl font-extrabold text-white mb-6 tracking-tight px-4 md:px-8">{title}</h2>
  ...
  if (items.length === 0) {
    return null; // Don't show the section if no items are found
  }

  const badgeColorClass = badgeColor || "bg-blue-600/20 text-blue-400 border-blue-500/30";
    ...
    <div className="mt-12 w-full mb-12">
      <div className="flex items-center justify-between px-4 md:px-8 mb-6">
        <h2 className="text-3xl font-extrabold text-white tracking-tight flex items-center gap-3">
          {title}
          <span className={`text-sm font-medium px-3 py-1 rounded-full border ${badgeColorClass}`}>
            {badgeText || "Trending"}
          </span>
        </h2>
      ...
      <div className="flex gap-6 overflow-x-auto px-4 md:px-8 pb-8 pt-2 scroll-smooth snap-x">
        ...
        {items.map((item, index) => (
          ...
          >
            {/* Rank Badge */}
            <div className="absolute -top-3 -left-3 z-20 bg-gray-800 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shadow-lg border-2 border-gray-900">
              {index + 1}
            </div>
